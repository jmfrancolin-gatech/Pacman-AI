...
class ExactInference(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        # instanciate a dictionary represanting a grid of the world
        grid = util.Counter()

        # if no noisyDistance is detected the ghost is in jail
        if noisyDistance == None:
            # update probability of jail postion to 1
            grid[self.getJailPosition()] = 1.0
        # ghost still in the grid
        else:
            # ghost still in the grid
            for pos in self.legalPositions:
                # get man_dist from pacman to grid position
                man_dist = util.manhattanDistance(pos, pacmanPosition)
                # update probability of ghost in grid posion
                grid[pos] = emissionModel[man_dist] * self.beliefs[pos]
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        # loop throgh the grid
        for oldPos in self.legalPositions:
            # get a new position distribution for each gris position given a game state
            newPosDist = self.getPositionDistribution(self.setGhostPosition(gameState, oldPos))
            # comsider all possible transitions
            for newPos, prob in newPosDist.items():
                # update grid of probabilities
                grid[newPos] += prob * self.beliefs[oldPos]
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        # instanciate self.particles
        self.particles = []

        # find number of particles per legal position ratio
        quotient =  (self.numParticles  / len(self.legalPositions))
        remainder = self.numParticles % len(self.legalPositions)

        # create a list of quotient instances of legalPositions coordinates
        for index in range(len(self.legalPositions)):
            particles = [self.legalPositions[index]] * quotient
            self.particles.append(particles)

            # take care of remaiders
            if remainder > 0:
                self.particles.append([self.legalPositions[index]])
                remainder = remainder - 1

        # flatten the list into a 1D array
        self.particles = sum(self.particles, [])
    ...
    def observe(self, observation, gameState):
        ...
        # get prob distribution of the grid
        grid = util.Counter()
        grid_old = self.getBeliefDistribution()
        for pos in self.legalPositions:
            man_distance = util.manhattanDistance(pos, pacmanPosition)
            grid[pos] = emissionModel[man_distance] * grid_old[pos]

        # if all probs are zero, resample uniformly
        if sum(grid.values()) == 0:
            self.initializeUniformly(gameState)
            return

        # resample based on current state of the grid
        self.particles = []
    ...
    def elapseTime(self, gameState):
        ...
        # create a pocy of the particles list
        particle_list = self.particles
        # reset particles list
        self.particles = []

        # sample a new particle pos from each curr particle pos based on the game state
        for pos in particle_list:
            positionDistribution = self.getPositionDistribution(self.setGhostPosition(gameState, pos))
            particle = util.sample(positionDistribution)
            self.particles.append(particle)
    ...
    def getBeliefDistribution(self):
        ...
        # initilalyze it as dictionary
        belivef_distribution = util.Counter()

        # translate list into dictionary of frequencies
        for particle in self.particles:
            belivef_distribution[particle]+= 1

        # nomalyze it and return
        belivef_distribution.normalize()
        return belivef_distribution
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        # reset particles
        self.particles = []
        # populate list with the randomized locations
        for _ in range(self.numParticles):
            for ghost in ghosts:
                self.particles.append(ghost)
    ...
    def getBeliefDistribution(self):
        ...
        # instamciat egrid
        grid = util.Counter()
        # loop throgh particles to populate grid
        for particle in self.particles:
            grid[particle] = grid[particle] + 1
        # normalyze it and retur
        grid.normalize()
        return grid
