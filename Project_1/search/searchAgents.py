    ...
    def getSuccessors(self, state):
        ...
        for action in [Directions.EAST, Directions.WEST, Directions.NORTH, Directions.SOUTH]:
            ...
            if (next_x, next_y) in self.corners:
                visited_corners[self.corners.index((next_x, next_y))] = True

            if not hitsWall:
                next_node = (next_x, next_y)
                next_state = (next_node, tuple(visited_corners))
...
def cornersHeuristic(state, problem):
    ...
    for i in range(len(corners)):
        goal_dist = util.manhattanDistance(curr_node, corners[i])
        if visited_corners[i] == True:
            goal_dist = 0

        man_dist.append(goal_dist)
...
def foodHeuristic(state, problem):
    ...
    for food_pos in food_list:
        goal_dist = util.manhattanDistance(position, food_pos)
        man_dist.append(goal_dist)
