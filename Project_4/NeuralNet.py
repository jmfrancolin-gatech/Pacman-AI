...
class Perceptron(object):
    ...
    def updateWeights(self, inActs, alpha, delta):
        ...
        for index in range(len(self.weights)):
            newWeight = self.weights[index] + (alpha * inActs[index] * delta)
            weights.append(newWeight)
            totalModification += abs(newWeight - self.weights[index])
        self.weights = weights
        inActs.pop(0)
...
class NeuralNet(object):
    ...
    def feedForward(self, inActs):
        ...
        for layer in self.layers:
            perceptrons = []
            for node in layer:
                perceptrons.append(node.sigmoidActivation(current))
            values.append(perceptrons)
            current = perceptrons
            ...
            for layerNum in xrange(self.numHiddenLayers-1,-1,-1):
                ...
                for neuronNum in xrange(len(layer)):
                    ...
                    for index in range(len(nextLayer)):
                        var += nextLayer[index].weights[neuronNum + 1] * deltas[0][index]
                    delta = gPrime * var
...
def buildNeuralNet(examples, alpha=0.1, weightChangeThreshold = 0.00008,hiddenLayerList = [1], maxItr = sys.maxint, startNNet = None):
    ...
    for example in examplesTest:
        nnOutputs = nnet.feedForward(example[0])[-1]
        nnOutputs = [round(elem) for elem in nnOutputs]

        if nnOutputs == example[1]:
            testCorrect += 1.0
        else:
            testError += 1.0
